...
# 2024-01-01 by Gaokeng Xiao, http://www.molcalx.com.cn
# extract tilte from docking docking results
#
def extract_title(docking_pdbqt_file):
    f = open(docking_pdbqt_file,'r')
    lines = f.readlines()
    f.close()
    # extract value of the scores
    title=[]
...
def add_title(sdf_string,title):
    ...
    for i in range(len(sdflines)):
        if 'RDKit' in sdflines[i]:
            sdflines[i-1] = title
    new_sdf_string='\n'.join(sdflines[0:len(sdflines)])
    return new_sdf_string
...
for filename in docking_results_filenames:
    ...
    # extract title
    mol_title = extract_title(filename)[0]
    print(mol_title)
    ...
    if template_filename is not None: # OBMol from template_filename
        ...
        output_string = add_title(output_string,mol_title)
        output_format = 'sdf'
        for i in failures:
